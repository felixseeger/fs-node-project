...
const FloatCard: FC<FloatCardProps> = ({ children, style, delay = 0 }) => {
...
const Toggle: FC<ToggleProps> = ({ label, description, defaultChecked = false }) => {
...
const Field: FC<FieldProps> = ({ label, value, mono, placeholder }) => {
  const [focused, setFocused] = useState(false);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{ fontSize: 12, fontWeight: 500, color: '#777' }}>{label}</label>
...
const ActionBtn: FC<ActionBtnProps> = ({ label, variant = 'ghost', onClick }) => {
  const [hovered, setHovered] = useState(false);
