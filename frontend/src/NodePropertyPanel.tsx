...
export default function NodePropertyPanel(): React.ReactElement | null {
  const selectedNodes = useStore((s: { nodes: Node[] }) => s.nodes.filter((n: Node) => n.selected));
  ...
  const updateNodeData = (key: string, value: unknown): void => {
    setNodes((nds: Node[]) =>
      nds.map((n: Node) => {
        if (n.id === node.id) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, borderBottom: `1px solid ${border.subtle}`, paddingBottom: 8 }}>
        <h3 style={{ margin: 0, ...font.md, color: text.primary }}>
          {(node.data as NodeData)?.label || node.type}
        </h3>
