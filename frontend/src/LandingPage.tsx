...
const NodeMini: FC<NodeMiniProps> = ({ label, id, color, top, left }) => {
  return (
    <div style={{
      position: 'absolute',
      left,
...
interface SparkleSmallProps {
  color?: string;
}

const SparkleSmall: FC<SparkleSmallProps> = ({ color = 'var(--color-text)' }) => {
  return (
...
// Sparkle icon for new workflow modal
...
interface NewWorkflowModalProps {
  onClose: () => void;
  onSelect: (type: string, aiPrompt?: string, aiMode?: string) => Promise<void>;
}

const NewWorkflowModal: FC<NewWorkflowModalProps> = ({ onClose, onSelect }) => {
  const [hovered, setHovered] = useState<string | null>(null);
  const [step, setStep] = useState<'pick' | 'ai'>('pick');
  const [aiPrompt, setAiPrompt] = useState('');
  const [aiMode, setAiMode] = useState<'fast' | 'creative'>('fast');
...
interface NewWorkflowItemProps {
  title: string;
  desc: string;
...
const NewWorkflowItem: FC<NewWorkflowItemProps> = ({ title, desc, icon, onClick, dataTestId }) => {
  return (
    <button
...
interface LandingPageProps {
  onCreateWorkflow: (name: string, id?: string | null, options?: any) => Promise<void>;
  onDeleteWorkflows: (ids: string[]) => Promise<void>;
  workflows?: any[];
  onNavigate: (page: string) => void;
...
export const LandingPage: FC<LandingPageProps> = ({ 
  ...
  const toggleTplSelection = (id: string) => {
  ...
  const handleModalSelect = async (type: string, aiPrompt?: string, aiMode?: string) => {
    ...
    <div id="landing-scroll" className="landing-page">
      ...
      <div style={{ width: '100%', position: 'relative' }}>
        ...
        <section style={{
          ...
          <div style={{
            background: 'linear-gradient(135deg, rgba(10, 10, 20, 0.8) 0%, rgba(20, 20, 30, 0.9) 100%)',
            ...
            <p style={{
              ...
              Ask questions, get explanations, and receive guidance directly from your AI pipelines
            </p>
              ...
              {[
                {
                  ...
                  desc: 'Ask questions in plain English about your workflow'
                },
                {
                  icon: '🔍',
                  title: 'Smart Analysis',
                  desc: 'Get intelligent insights about node connections and data flow'
                },
                {
                  icon: '🎯',
                  title: 'Actionable Guidance',
                  desc: 'Receive specific recommendations for optimization'
                }
              ].map((feature, idx) => (
                <div key={idx} style={{
                ...
                onMouseLeave={(e) => {
                  ...
                  <p style={{ fontSize: 14, color: 'var(--color-text-dim)', lineHeight: 1.5 }}>
                    {feature.desc}
                  </p>
                </div>
              ))}
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="white">
                    <path d="M21 16v-2a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2" />
                    <path d="M16 8V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2" />
                    <path d="M12 12l3-3m0 0l-3-3m3 3h-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" />
                  </svg>
                </div>
                <div style={{ fontSize: 18, fontWeight: 500, color: 'var(--color-text-dim)' }}>
                  Try asking: "How does this workflow process images?"
                </div>
              </div>
            </div>
            ...
            <p style={{
              ...
              Powered by AI-driven natural language understanding
            </p>
          </div>
        </section>
            ...
            <div style={{
              ...
              {workflows.filter(w => !w.deleted).map((wf) => (
                <div
                  ...
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--color-accent)'; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = selectedWfs.has(wf.id) ? 'var(--color-accent)' : 'var(--color-border)'; }}
                >
                  {/* Mini preview */}
                  <div
                    style={{
              ...
              <div style={{
                ...
                {templates.map((tpl) => (
                  <div
                    ...
                    onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--color-accent)'; }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = selectedTpls.has(tpl.id) ? 'var(--color-accent)' : 'var(--color-border)'; }}
                  >
                    {isSelectingTemplates && (
                      <div style={{
                        position: 'absolute', top: 12, right: 12, width: 16, height: 16,
