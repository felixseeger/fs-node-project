...
import { border, text, ui, sp, radius, font } from './nodeTokens';

/**
 * Section header with an attached ReactFlow handle.
 * Replaces the duplicated sectionHeader() helper across all nodes.
 */
export function SectionHeader({ label, handleId, handleType, color, extra }) {
  return (
    <div
      style={{
 ...
 */
export function LinkedBadges({ nodeId, handleId, onUnlink }) {
  return (
    <>
      <span
        style={{
 ...
 */
export function ConnectedOrLocal({ connected, connInfo, children }) {
