...
export default function ImageAnalyzerNode({ id, data, selected }) {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    const prompt = data.resolveInput?.(id, 'prompt-in') || data.localPrompt || '';
    const systemDirections = data.resolveInput?.(id, 'system-in') || data.systemDirections || '';
  ...
  // Image grid: show uploaded images + empty "+" slots up to 4
  const maxSlots = 4;
  ...
  return (
    ...
    >
      ...
      {sectionHeader('Prompt', 'prompt-in', 'target', getHandleColor('prompt-in'),
        () => data.onAddToInput?.('prompt', id, 'prompt-in')
      )}
      <textarea
      ...
      {sectionHeader('System Prompt', 'system-in', 'target', getHandleColor('system-in'),
        () => data.onAddToInput?.('text', id, 'system-in')
      )}
      <div style={{ position: 'relative' }}>
        <textarea
          value={data.systemDirections || ''}
          onChange={(e) => update({ systemDirections: e.target.value })}
      ...
      <div style={{
        ...
        {isAnalyzing
          ...
          : (data.analysisResult || <span style={{ color: '#555' }}>Analysis output will appear here...</span>)
        }
      </div>
