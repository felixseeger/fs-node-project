...
export default function UniversalSimplifiedNodeChrome({
  ...
  const modal = commentModalOpen
    ? createPortal(
        <div
          className="nodrag nopan" role="presentation"
          style={{ position: 'fixed', inset: 0, zIndex: 100002, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: sp[6] }}
          onMouseDown={(e) => { if (e.target === e.currentTarget) toggleCommentModal(false); }}
        >
          <div role="dialog" aria-labelledby="universal-node-comment-heading" aria-describedby="universal-node-comment-desc"
            style={{ width: 'min(400px, 100%)', background: surface.deep, border: `1px solid ${border.default}`, borderRadius: radius.lg, padding: sp[5], boxShadow: '0 16px 48px rgba(0,0,0,0.5)' }}
          >
            <h2 id="universal-node-comment-heading" style={{ position: 'absolute', width: 1, height: 1, padding: 0, margin: -1, overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap', border: 0 }}>Node comment</h2>
            <span id="universal-node-comment-desc" className="sr-only" style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' }}>Optional note for this node</span>
            <textarea id="universal-node-comment-body" className="nodrag nopan" aria-label="Add a comment" value={draftComment} onChange={(e) => setDraftComment(e.target.value)} placeholder="Add a comment..." rows={4}
              style={{ width: '100%', resize: 'vertical', minHeight: 96, background: 'rgba(0,0,0,0.35)', border: `1px solid ${border.subtle}`, borderRadius: radius.md, color: text.primary, ...font.sm, padding: sp[3], outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: sp[3], marginTop: sp[4] }}>
              <button type="button" className="nodrag nopan" onClick={() => toggleCommentModal(false)} style={{ background: 'transparent', border: 'none', color: text.secondary, ...font.sm, cursor: 'pointer', padding: `${sp[2]}px ${sp[3]}px` }}>Cancel</button>
              <button type="button" className="nodrag nopan" onClick={saveComment} style={{ background: border.active, border: 'none', color: '#fff', ...font.sm, fontWeight: 600, cursor: 'pointer', padding: `${sp[2]}px ${sp[4]}px`, borderRadius: radius.md }}>Save</button>
            </div>
          </div>
        </div>,
        document.body
      ...
      <div style={{ position: 'relative', width, overflow: 'visible' }} data-capabilities={capabilities.join(',')} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
        ...
        {showActions && (
          <div style={{ position: 'absolute', top: -44, left: 0, display: 'flex', alignItems: 'center', gap: sp[2], opacity: showActions ? 1 : 0, pointerEvents: showActions ? 'auto' : 'none', transition: 'opacity 0.12s ease', zIndex: 10 }}>
            ...
            {onDownload && (
              <button type="button" title="Download" className="nodrag nopan" onClick={(e) => { e.stopPropagation(); onDownload(); }}
                style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, border: `1px solid ${border.default}`, background: surface.base, color: text.secondary, cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}
              >
