...
export default function PixVerseV5Node({ id, data, selected }) {
  ...
  const localMotionIntensity = data.localMotionIntensity || 5;
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const params = {
        image: images[0],
        prompt,
        aspect_ratio: localRatio,
        resolution: localResolution,
        motion_intensity: localMotionIntensity,
      };
      ...
      if (taskId) {
        const status = await pollPixVerseV5Status(taskId);
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 11, color: '#999' }}>Motion Intensity</span>
            <span style={{ fontSize: 11, color: ACCENT, fontWeight: 600 }}>{localMotionIntensity}</span>
          </div>
          <input type="range" min="1" max="10" step="1" value={localMotionIntensity}
            onChange={(e) => update({ localMotionIntensity: Number(e.target.value) })}
