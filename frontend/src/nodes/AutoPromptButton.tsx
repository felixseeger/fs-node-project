...
function asImageArray(value: unknown): string[] | undefined {
...
export interface AutoPromptButtonProps {
  id: string;
...
export default function AutoPromptButton({
  id,
  data,
  update,
  imageKey = 'image-in',
  localImageKey = 'localImage',
  promptKey = 'inputPrompt',
}: AutoPromptButtonProps) {
  const [isGenerating, setIsGenerating] = useState(false);

  const handleAutoPrompt = async () => {
    let images = asImageArray(data.resolveInput?.(id, imageKey));
    if (!images?.length) {
      images = asImageArray(data[localImageKey]);
    }
    ...
    try {
      ...
      if (result.error) {
        throw new Error(
          result.error?.message || JSON.stringify(result.error) || 'Error generating prompt'
        );
      }
  ...
  return (
    <button
      ...
      style={{
        fontSize: 9,
        color: isGenerating ? '#999' : '#f59e0b',
        padding: '2px 6px',
        background: isGenerating ? '#333' : 'rgba(245,158,11,0.15)',
        borderRadius: 4,
        border: `1px solid ${isGenerating ? '#444' : 'rgba(245,158,11,0.3)'}`,
    ...
    >
      <svg
        ...
        viewBox="0 0 24 24"
        fill="none"
        stroke={isGenerating ? '#999' : '#f59e0b'}
        strokeWidth="2"
        strokeLinecap="round"
