...
import { createElement, lazy, ComponentType, LazyExoticComponent } from 'react';
 ...
 * @param {LazyExoticComponent<ComponentType<any>> | ComponentType<any>} NodeComponent - The component
 * @returns {ComponentType<NodeProps>} - Wrapper component ready for React Flow registration
 */
export function createDynamicNodeWrapper(NodeComponent: LazyExoticComponent<ComponentType<any>> | ComponentType<any>): ComponentType<NodeProps> {
  ...
  return function DynamicNodeWrapper(props: NodeProps) {
    return createElement(DynamicNodeLoader, {
      ...
      LazyComponent: (NodeComponent as any).$$typeof === Symbol.for('react.lazy') 
        ? NodeComponent 
...
const Kling3Node = lazy(() => (import('../nodes/Kling3Node') as any).catch(error => {
