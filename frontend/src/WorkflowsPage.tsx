...
const SparkleSmall: FC<SparkleSmallProps> = ({ color = '#fff' }) => {
  return (
...
const NewWorkflowModal: FC<NewWorkflowModalProps> = ({ onClose, onSelect }) => {
  ...
  const [aiMode, setAiMode] = useState<'fast' | 'pro'>('fast');
...
interface WorkflowsPageProps {
  ...
  workflows?: (Workflow & { deleted?: boolean, nodeCount?: number })[];
}
          ...
          <div style={{ flex: 1, maxWidth: 650, zIndex: 1, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
              <button
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 0 32px rgba(59, 59, 255, 0.6)'; (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(-1px)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 0 24px rgba(59, 59, 255, 0.4)'; (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(0)'; }}
              >
                Start building
              </button>
              ...
              <button
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.05)'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#666'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#444'; }}
              >
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#4a4aff'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#3B3BFF'; }}
              >
                Deploy as API
              </button>
            </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {isSelecting ? (
              <>
                ...
                >
                  {selectedWfs.size === workflows.filter(w => !w.deleted).length && workflows.filter(w => !w.deleted).length > 0 ? 'Deselect All' : 'Select All'}
                </button>
                <button
        ...
        <div style={{
          ...
          {workflows.filter(w => !w.deleted).map((wf) => (
            <div
              ...
              onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#3b82f6'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = selectedWfs.has(wf.id) ? '#3b82f6' : '#2a2a2a'; }}
            >
              {/* Mini preview */}
              <div
                style={{
          ...
          <div style={{
            ...
            {templates.map((tpl) => (
              <div
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#3b82f6'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = selectedTpls.has(tpl.id) ? '#3b82f6' : '#2a2a2a'; }}
              >
                {isSelectingTemplates && (
                  <div style={{
                    position: 'absolute', top: 12, right: 12, width: 16, height: 16,
        ...
        <div style={{ marginTop: 56, marginBottom: 60 }}>
          ...
          <div style={{
            ...
            <div
              ...
              onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#ef4444'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#2a2a2a'; }}
            >
              <div style={{
                width: 40, height: 40, borderRadius: 10,
