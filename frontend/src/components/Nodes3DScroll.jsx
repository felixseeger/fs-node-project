...
export default function Nodes3DScroll() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Rect.top is relative to viewport. Correct for progress.
      const progress = -rect.top / (totalHeight - window.innerHeight);
      state.targetScroll = Math.max(0, progress) * (loopSize / CONFIG.camSpeed);

      // Track raw scroll for velocity
      const currentY = scrollParent === window ? window.scrollY : scrollParent.scrollTop;
    ...
    function raf(time) {
      ...
      // Smooth scroll lerping for camera
      const lastX = state.scroll;
      state.scroll += (state.targetScroll - state.scroll) * 0.08;
      
      // Calculate velocity from the delta of our smoothed scroll
