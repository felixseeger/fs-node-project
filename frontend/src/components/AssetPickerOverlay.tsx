...
const AssetPickerOverlay: FC<AssetPickerOverlayProps> = ({ isOpen, onClose, assets, canvasImages = [], onSelectImage }) => {
  ...
  return (
    ...
    >
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#eee' }}>Pick reference image</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        {canvasImages.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10, color: '#ec4899', marginBottom: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>On Canvas</div>
        ...
        {assets.length === 0 && canvasImages.length === 0 ? (
          <div style={{ padding: '20px 0', textAlign: 'center', color: '#666', fontSize: 12 }}>
            No images found on canvas or in library.
          </div>
        ) : assets.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ fontSize: 10, color: '#888', marginBottom: -8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>From Library</div>
            {assets.map((asset) => (
              <div key={asset.id}>
