...
export default function GlobalProgressBar({ nodes }) {
  ...
  return (
    <div style={{
      ...
      {activeNodes.map(node => {
        ...
        const error = node.data?.outputError || node.data?.executionError || (node.data?.executionStatus === 'failed' ? message : null);
        ...
        return (
          <div key={node.id} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#e0e0e0', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, maxWidth: '60%' }}>
                <span style={{ color: error ? '#ef4444' : '#9ca3af', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {error ? `Error: ${error}` : message} {progress > 0 && !error ? `(${Math.round(progress)}%)` : ''}
                </span>
                {error && (
