...
interface GooeyNodesMenuProps {
  nodeMenu: NodeMenuSection[];
  templates?: Template[];
  assets?: Asset[];
  publicAssets?: Asset[];
  onCreateAsset?: (data: any) => void;
  onAddNode: (type: string, defaults: any) => void;
  onOpenProfile?: () => void;
}
...
const Icons = {
  ...
  TwitterX: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.901 1.153h3.68l-804 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" /></svg>,
...
const CATEGORIES = [
  ...
  { id: 'Assets', icon: Icons.Hash, title: 'Library' },
  { id: 'Community', icon: Icons.Globe, title: 'Community' },
  { id: 'History', icon: Icons.History, title: 'History' },
  { id: 'TextElement', icon: Icons.Text, title: 'Add Text' },
...
export const GooeyNodesMenu: FC<GooeyNodesMenuProps> = ({ 
  nodeMenu, 
  templates = [], 
  assets = [], 
  onCreateAsset, 
  ...
  const allNodes = useMemo(() => {
    return nodeMenu.reduce((acc: any[], section) => {
  ...
  }, [activeCategory, searchQuery, allNodes, nodeMenu, templates]);
    ...
    <div className="ms-menu-wrapper" onMouseLeave={() => { setActiveSubMenu(null); setShowHelpMenu(false); }}>
                ...
                <a
                  ...
                  onClick={(e) => {
                    ...
                    if (cat.id === 'History') {
                      ...
                      setActiveCategory(null);
                      setShowHelpMenu(false);
                    } else if (cat.id === 'TextElement') {
                      onAddNode('textElement', { text: '<p>Double click or right-click to edit text...</p>' });
                      setIsOpen(false);
                      setActiveCategory(null);
                      setShowHelpMenu(false);
                    } else {
      ...
      <div className={`ms-overlay-panel ${isOpen ? 'active' : ''}`}>
        ...
        <div className="ms-overlay-scroll-area">
          ...
          ) : activeCategory === 'Community' && !searchQuery ? (
            <div className="ms-assets-browser">
              <div className="ms-assets-section">
                <div className="ms-assets-title">Public Library <span>{publicAssets?.length || 0}</span></div>
                <div className="ms-assets-grid">
      ...
      {/* Profile Modal */}
      {/* Asset Modal */}
      <UpdateAssetModal isOpen={showAssetModal} onClose={() => setShowAssetModal(false)} onCreateAsset={(data: any) => {
