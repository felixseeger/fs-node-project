...
(async () => {
  ...
  appCode = appCode.replace(
    /const unsubscribeAuth = \(\) => \{\};\n\s+setAuthLoading\(false\);\n\s+setIsAuthenticated\(true\);\n\s+setCurrentUserId\("testuser@nodeproject.dev"\);\n\s+setCurrentUserEmail\("testuser@nodeproject.dev"\);\n\s+initializeProfile\("testuser@nodeproject.dev", "testuser@nodeproject.dev", "Test User"\);\n\s+if \(\!sessionStorage\.getItem\(SLP_KEY\)\) \{\n\s+setShowSystemLoading\(true\);\n\s+\}/g,
    `const unsubscribeAuth = () => {};
  ...
  try {
    ...
    await pageA.evaluate(() => {
        ...
        const cb = btns.find(b => b.title?.includes('Collaboration') || b.textContent?.includes('Collaboration'));
        if (cb) cb.click();
    });
        ...
        if (container) {
          container.innerHTML = `
            <div style="display: flex; flex-direction: column; gap: 12px; padding-bottom: 20px;">
              <div style="display: flex; flex-direction: row-reverse; gap: 8px; align-self: flex-end; justify-content: flex-end; max-width: 80%;">
