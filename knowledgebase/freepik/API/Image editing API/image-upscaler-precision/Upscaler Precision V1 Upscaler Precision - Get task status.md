...
````yaml get /v1/ai/image-upscaler-precision/{task-id}
openapi: 3.0.0
info:
  description: >-
...
security:
  - apiKey: []
paths:
  /v1/ai/image-upscaler-precision/{task-id}:
    get:
      tags:
        - upscaler-precision
      summary: Upscaler Precision - Get task status
      description: >-
        Returns the current status and output URL of a specific precision
        upscaler task. The output URL is included only if the task has completed
        successfully.
      operationId: getAiImageUpscalerPrecisionTask
      parameters:
