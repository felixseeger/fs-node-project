...
````yaml get /v1/ai/image-change-camera/{task-id}
openapi: 3.0.0
info:
  description: >-
...
security:
  - apiKey: []
paths:
  /v1/ai/image-change-camera/{task-id}:
    get:
      tags:
        - image-change-camera
      summary: Change Camera - Get task status
      description: >-
        Retrieve the status and result of a specific Change Camera task by its
        task ID. When the task status is `completed`, the response includes the
        URL of the generated image with the new camera angle.
      operationId: get_change_camera_task
      parameters:
