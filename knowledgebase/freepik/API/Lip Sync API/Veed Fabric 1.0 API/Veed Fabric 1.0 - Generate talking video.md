...
````yaml post /v1/ai/lip-sync/veed-fabric-1-0
openapi: 3.0.0
info:
  description: >-
...
paths:
  /v1/ai/lip-sync/veed-fabric-1-0:
    post:
      tags:
        - lip-sync
      summary: Veed Fabric 1.0 - Generate talking video
      description: >
        Generate a realistic talking video by combining a static portrait image
        with an audio file using Veed Fabric 1.0.

        The model produces a lip-synced video (MP4) where the person in the
        image speaks naturally in sync with the provided audio.

        Ideal for creating talking head videos from a single photo and voice
        recording.
      operationId: create_veed_fabric_1_0_lip_sync
      requestBody:
        content:
          application/json:
            examples:
              all-params:
                $ref: '#/components/examples/request-veed-fabric-1-0-all-params'
              required-params:
                $ref: '#/components/examples/request-veed-fabric-1-0-required-params'
            schema:
...
components:
  examples:
    request-veed-fabric-1-0-all-params:
      summary: Request - Veed Fabric 1.0 all params
      value:
        resolution: 720p
        image_url: https://example.com/portrait.jpg
        audio_url: https://example.com/speech-audio.mp3
        webhook_url: https://my-webhook-url.com/endpoint
    request-veed-fabric-1-0-required-params:
      summary: Request - Veed Fabric 1.0 required params
      value:
        resolution: 720p
        image_url: https://example.com/portrait.jpg
        audio_url: https://example.com/speech-audio.mp3
