...
````yaml get /v1/ai/lip-sync/veed-fabric-1-0-fast/{task-id}
openapi: 3.0.0
info:
  description: >-
...
security:
  - apiKey: []
paths:
  /v1/ai/lip-sync/veed-fabric-1-0-fast/{task-id}:
    get:
      tags:
        - lip-sync
      summary: Veed Fabric 1.0 Fast - Get task status
      description: >-
        Retrieve a specific Veed Fabric 1.0 Fast lip-sync task by its ID,
        including generation status and result URL when completed.
      operationId: get_veed_fabric_1_0_fast_task_status
      parameters:
            ...
            application/json:
              examples:
                success - completed task:
                  $ref: >-
                    #/components/examples/200-veed-fabric-1-0-fast-task-completed
...
components:
  examples:
    200-veed-fabric-1-0-fast-task-completed:
      summary: Success - Veed Fabric 1.0 Fast task completed
      value:
        data:
          generated:
            - >-
              https://ai-statics.freepik.com/lip-sync-veed-fabric-1-0-fast-output.mp4
