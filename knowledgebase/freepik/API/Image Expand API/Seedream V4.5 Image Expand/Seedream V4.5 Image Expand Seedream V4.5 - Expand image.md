...
````yaml post /v1/ai/image-expand/seedream-v4-5
openapi: 3.0.0
info:
  description: >-
...
paths:
  /v1/ai/image-expand/seedream-v4-5:
    post:
      tags:
        - image-expand
      summary: Seedream V4.5 - Expand image
      description: >
        This endpoint allows you to expand an image using the Seedream V4.5 AI
        model. The image will be expanded based on the provided pixel values for
        each side.

        If no prompt is provided, the model will auto-generate one based on the
        image content.
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/image-expand-seedream-v4-5-request'
        required: true
      responses:
        '200':
          content:
...
components:
  schemas:
    image-expand-seedream-v4-5-request:
      properties:
        webhook_url:
          description: >
        ...
        image:
          description: >-
            Image to expand. Accepts an HTTPS URL or a Base64-encoded image (max
            10MB).
          type: string
        prompt:
          description: >-
            Optional text prompt describing the desired expanded content. If not
            provided, the AI model will auto-generate a prompt based on the
            image.
          type: string
        left:
          description: Number of pixels to expand on the left side
          maximum: 2048
          minimum: 0
          type: integer
        right:
          description: Number of pixels to expand on the right side
          maximum: 2048
          minimum: 0
          type: integer
        top:
          description: Number of pixels to expand on the top side
          maximum: 2048
          minimum: 0
          type: integer
        bottom:
          description: Number of pixels to expand on the bottom side
          maximum: 2048
          minimum: 0
          type: integer
        seed:
