...
export async function errorHandler(err, req, res, next) {
  ...
  // Refund credits if deducted but request failed
  if (req.creditInfo && req.creditInfo.deducted) {
    try {
      if (req.user && req.user.uid) {
        console.log(`[Refund] Refunding ${req.creditInfo.deducted} credits to user ${req.user.uid} due to failed request`);
        await addCredits(req.user.uid, req.creditInfo.deducted);
      }
    } catch (refundError) {
