...
export async function generateImage(params) {
  ...
  try {
    ...
    if (generated.length === 0) {
      throw new Error('Failed to parse generated images from Google API response');
    }
 ...
 */
export async function generateText({ prompt, system, history = [], images = [], maxTokens = 2048, jsonMode = false }) {
  const apiKey = process.env.GOOGLE_GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GOOGLE_GEMINI_API_KEY is not set');
  }
  ...
  for (const m of history) {
    ...
    if (chatHistory.length === 0 && role === 'model') continue; // drop leading model msg
