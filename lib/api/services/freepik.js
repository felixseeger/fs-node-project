...
function getHeaders() {
  if (!FREEPIK_API_KEY) {
    console.error('[Freepik] Warning: FREEPIK_API_KEY is not set');
  }
  return {
...
export async function generateImage(params) {
  ...
  // Refined model mapping for Mystic API
  // Note: For /v1/ai/mystic, model is optional and specific values are expected.
  // We'll map 'kora' to 'automatic' or omit completely as default.
  if (model) {
    if (model === 'kora' || model === 'freepik-mystic') {
      // For the mystic endpoint, it's often better to let it auto-select or specify a valid engine.
      // Based on documentation, the mystic endpoint doesn't always need 'model' if it's already the mystic endpoint.
      // We can use 'automatic' as a safe bet if model was explicitly requested.
      body.model = 'automatic';
    } else {
      const validModels = ['automatic', 'Illusio', 'Sharpy', 'Sparkle', 'realism', 'editorial_portraits'];
  ...
  console.log('[Freepik] Prepared body for image generation:', { ...body, structure_reference: body.structure_reference ? '(base64...)' : undefined });
