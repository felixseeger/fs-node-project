...
import * as luma from '../services/luma.js';
import * as minimax from '../services/minimax.js';
import * as topazVideo from '../services/topazVideo.js';
...
router.post('/ltx-direct/:mode', generationLimiter, async (req, res, next) => {
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ error: 'LTX_API_KEY not configured' });
    }

    console.log(`[API] LTX Direct ${mode} generation with params:`, JSON.stringify(req.body, null, 2));
    ...
    console.log(`[API] LTX Direct response status: ${response.status}`);
    ...
    if (!response.ok) {
      ...
      console.error('[API] LTX Direct failed:', {
    ...
    // LTX text-to-video returns binary video directly on success (200)
    // LTX image-to-video may return JSON with request_id for async processing
    const contentType = response.headers.get('content-type');
    console.log('[API] LTX Direct response content-type:', contentType);

    if (contentType && (contentType.includes('application/octet-stream') || contentType.includes('video/'))) {
      // Binary video response - send it directly to the client
      console.log('[API] LTX Direct: Returning binary video stream');
      res.setHeader('Content-Type', contentType || 'video/mp4');
      const buffer = await response.arrayBuffer();
      res.send(Buffer.from(buffer));
    } else {
      // JSON response - expect async task with request_id
      const responseData = await response.json();
      console.log('[API] LTX Direct response data:', responseData);

      const taskId = responseData.request_id;
      if (!taskId) {
        console.error('[API] LTX Direct: No request_id in response', responseData);
        return res.status(500).json({
          error: { message: 'LTX API did not return request_id', details: responseData }
        });
      }
    ...
    console.error('[API] LTX Direct failed:', err.message, err.stack);
    next(err);
  }
});

// --- LTX Video Direct Status Polling ---
router.get('/ltx-direct/:mode/:taskId', async (req, res, next) => {
  try {
    ...
    console.log(`[API] LTX Direct status check for ${taskId}`);

    const response = await fetch(`https://api.ltx.video/v1/${mode}/${taskId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
      },
    ...
    if (!response.ok) {
      ...
      console.error('[API] LTX Direct status failed:', { status: response.status, error: errorData });
      return res.status(response.status).json(errorData);
    }

    const responseData = await response.json();
    console.log('[API] LTX Direct status response:', responseData);

    // Map LTX status to our format
    let status = 'PENDING';
    let generated = [];

    if (responseData.status === 'completed') {
      status = 'COMPLETED';
      // LTX returns download_url for the video
      if (responseData.download_url) {
        generated = [responseData.download_url];
      }
    ...
    console.error('[API] LTX Direct status check failed:', err.message);
    next(err);
  }
});

// --- Luma Reframe Video ---
router.post('/luma-reframe', generationLimiter, async (req, res, next) => {
  try {
    console.log('[API] Luma Reframe video');
    const result = await generateQueue.add(() => luma.reframeVideo(req.body));
    res.json(result);
  } catch (err) {
    console.error('[API] Luma Reframe failed:', err.message);
    next(err);
  }
});

// --- Minimax Video-01 ---
router.post('/minimax-video-01', generationLimiter, async (req, res, next) => {
  try {
    console.log('[API] Minimax Video-01 generation');
    const result = await generateQueue.add(() => minimax.generateVideo(req.body));
    res.json(result);
  } catch (err) {
    console.error('[API] Minimax Video-01 failed:', err.message);
    next(err);
  }
});
...
// --- Topaz Video Upscale ---
router.post('/topaz-video-upscale', generationLimiter, async (req, res, next) => {
  try {
    console.log('[API] Topaz video upscale');
    const result = await generateQueue.add(() => topazVideo.upscaleVideo(req.body));
    res.json(result);
  } catch (err) {
    console.error('[API] Topaz video upscale failed:', err.message);
    next(err);
  }
});
