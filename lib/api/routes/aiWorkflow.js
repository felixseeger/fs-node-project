...
const TERMINAL_NODE_SPECS = {
  image: { type: 'imageOutput', label: 'Image Output', targetHandle: 'image-in' },
  video: { type: 'videoOutput', label: 'Video Output', targetHandle: 'video-in' },
  audio: { type: 'soundOutput', label: 'Sound Output', targetHandle: 'audio-in' },
};

function inferMediaTypeFromNodeType(type) {
  if (VIDEO_NODE_TYPES.has(type)) return 'video';
  if (AUDIO_NODE_TYPES.has(type)) return 'audio';
  return 'image';
}

function inferMediaTypeFromNodes(nodes = []) {
...
function inferMediaTypeFromIntent(intent) {
  const categories = Array.isArray(intent?.categories) ? intent.categories : [];
  if (categories.some((category) => String(category).toLowerCase().includes('video'))) return 'video';
  if (categories.some((category) => String(category).toLowerCase().includes('audio'))) return 'audio';

  const preferredType = intent?.preferredNode?.type;
  if (preferredType) return inferMediaTypeFromNodeType(preferredType);
  return 'image';
}

function createTerminalNode(mediaType = 'image', label) {
  const spec = TERMINAL_NODE_SPECS[mediaType] || TERMINAL_NODE_SPECS.image;
  return {
    type: spec.type,
...
function createTerminalNodeForIntent(intent, label) {
  return createTerminalNode(inferMediaTypeFromIntent(intent), label);
}

function getTerminalTargetHandle(nodeType) {
  const mediaType = inferMediaTypeFromNodeType(nodeType);
  return TERMINAL_NODE_SPECS[mediaType]?.targetHandle || 'image-in';
}
...
const WORKFLOW_PATTERNS = [
  {
    ...
    nodes: (intent) => [
      ...
      createTerminalNode('image', 'Image Output'),
    ],
  },
  {
    name: 'Image to Video',
    ...
    nodes: (intent) => [
      ...
      createTerminalNode('video', 'Video Output'),
    ],
  },
  {
  ...
  },
  {
    ...
    nodes: (intent) => [
      ...
      createTerminalNode('audio', 'Sound Output'),
    ],
  },
  {
  ...
  },
  {
    ...
    nodes: (intent) => {
      ...
      nodes.push(createTerminalNodeForIntent(intent, 'Image Output'));
      return nodes;
    },
  },
];
  ...
  for (const provider of providers) {
    try {
      ...
      const nodeDefinitions = [
        ...
        ...parsed.nodes,
        createTerminalNode(inferMediaTypeFromNodes(parsed.nodes)),
      ];
      if (provider.name === 'Anthropic') {
        console.log('[AI Workflow] Anthropic fallback successful');
      }
      return { name: parsed.name, nodeDefinitions };
    } catch (err) {
      lastError = err;
      console.warn(`[AI Workflow] ${provider.name} failed for workflow generation:`, err.message);
    }
  }

  throw new Error(`Both AI providers failed: ${lastError?.message || 'unknown error'}`);
}
...
function buildVirtualTryOnWorkflow() {
  ...
  const edges = [
    ...
    // Generator → Response
    { id: `e-${generatorId}-${responseId}`, source: generatorId, target: responseId, sourceHandle: 'output', targetHandle: 'image-in', animated: true },
  ];
...
function autoLayoutNodes(nodes, options = {}) {
  ...
  const outputNodes = nodes.filter(n => ['response', 'output', 'imageOutput', 'videoOutput', 'soundOutput'].includes(n.type));
  const processingNodes = nodes.filter(n => n.type !== 'inputNode' && !['response', 'output', 'imageOutput', 'videoOutput', 'soundOutput'].includes(n.type));

  // Determine inputNode fields based on processing nodes
  ...
  // Only include fields that have corresponding target handles on processing nodes
  const NODES_WITH_ASPECT_RATIO_IN = new Set(['fluxReimagine', 'creativeUpscale', 'precisionUpscale']);
  const hasAspectTarget = processingNodes.some(n => NODES_WITH_ASPECT_RATIO_IN.has(n.type));

  const initialFields = inputFieldInfo.field === 'image_urls'
    ? (hasAspectTarget ? ['image_urls', 'aspect_ratio'] : ['image_urls'])
    : (hasAspectTarget ? ['prompt', 'aspect_ratio'] : ['prompt']);
...
export function generateEdges(nodes) {
  const edges = [];
  const terminalTypes = new Set(['response', 'output', 'imageOutput', 'videoOutput', 'soundOutput']);
  const processingNodes = nodes.filter(n => n.type !== 'inputNode' && !terminalTypes.has(n.type));
  const inputNode = nodes.find(n => n.type === 'inputNode');
  const outputNode = nodes.find(n => terminalTypes.has(n.type));

  // Nodes that accept an aspect-ratio-in handle from inputNode
  const ASPECT_RATIO_TARGET_NODES = new Set(['fluxReimagine', 'creativeUpscale', 'precisionUpscale']);
  ...
  if (inputNode && processingNodes.length > 0) {
    ...
    // Connect aspect_ratio from inputNode to the first processing node that accepts it
    const aspectTarget = processingNodes.find(n => ASPECT_RATIO_TARGET_NODES.has(n.type));
    if (aspectTarget) {
      edges.push({
        id: `e-${inputNode.id}-aspect_ratio-${aspectTarget.id}`,
        source: inputNode.id,
        target: aspectTarget.id,
        sourceHandle: 'aspect_ratio',
        targetHandle: 'aspect-ratio-in',
        animated: true,
      });
    }
  }
  ...
  if (outputNode && processingNodes.length > 0) {
    ...
    const terminalHandle = getTerminalTargetHandle(outputNode.type);
    edges.push({
      id: `e-${last.id}-${outputNode.id}`,
      source: last.id,
      target: outputNode.id,
      sourceHandle: SOURCE_HANDLE[last.type] || 'output',
      targetHandle: terminalHandle,
      animated: true,
    });
  }
...
router.post('/ai-workflow/generate', generationLimiter, async (req, res) => {
  try {
    ...
    console.error('[AI Workflow] Error generating workflow:', error?.stack || error);
    res.status(500).json({
      success: false,
