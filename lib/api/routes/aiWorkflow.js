 ...
 */
async function generateWorkflowWithClaude(prompt, mode) {
  const model = mode === 'pro' ? 'claude-sonnet-4-6' : 'claude-haiku-4-5-20251001';
...
function buildVirtualTryOnWorkflow() {
  ...
  const edges = [
    ...
    { id: `e-${clothingId}-${visionId}`, source: clothingId, target: visionId, sourceHandle: 'output', targetHandle: 'image-in', animated: true },
    // Model → Claude Haiku
    { id: `e-${modelId}-${haikuId}`, source: modelId, target: haikuId, sourceHandle: 'output', targetHandle: 'image-in', animated: true },
    // Claude Vision → Generator
    { id: `e-${visionId}-${generatorId}`, source: visionId, target: generatorId, sourceHandle: 'image-out', targetHandle: 'prompt-in', animated: true },
    // Claude Haiku → Generator
    { id: `e-${haikuId}-${generatorId}`, source: haikuId, target: generatorId, sourceHandle: 'image-out', targetHandle: 'prompt-in', animated: true },
    // Generator → Response
...
router.post('/ai-workflow/generate', generationLimiter, async (req, res) => {
  try {
    ...
    if (isVirtualTryOnPrompt(prompt)) {
      ...
      let nodeDefinitions;
      try {
        const result = await generateWorkflowWithClaude(prompt, mode);
