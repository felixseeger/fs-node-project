...
router.post('/improve-prompt', generationLimiter, async (req, res, next) => {
  try {
    ...
    // Freepik improvePrompt returns the improved prompt directly
    // Format response to match expected structure
    if (result?.data?.improved_prompt) {
      res.json({
        data: {
          generated: [result.data.improved_prompt],
          status: 'COMPLETED'
        }
      });
      ...
      res.json({
        data: {
          generated: [result.improved_prompt],
          status: 'COMPLETED'
        }
      });
    } else {
