...
// Wrap provider tests with timeout to prevent hanging on Vercel (10s limit)
const testWithTimeout = (promise, timeoutMs = 2000) =>
  Promise.race([
    promise,
...
router.get('/debug/providers', async (req, res) => {
  ...
  // Test Google
  try {
    const googleResult = await testWithTimeout(
  ...
  // Test Replicate
  try {
    const replicateResult = await testWithTimeout(
  ...
  // Test Ideogram
  try {
    const ideogramResult = await testWithTimeout(
      ideogram.generateImage({
        prompt: 'red square',
  ...
  try {
    const klingResult = await testWithTimeout(
      fal.generateKlingVideo({
  ...
  try {
    const heygenResult = await testWithTimeout(
      heygen.generateHeyGenVideo({
  ...
  // Test Ernie (text-to-image)
  try {
    const ernieResult = await testWithTimeout(
      ernie.generateImage({
    ...
    results.providers.ernie = {
      status: 'OK',
      response: ernieResult.data?.[0]?.url ? '✓ URL returned' : '✗ No URL'
    };
  } catch (err) {
    results.providers.ernie = {
      status: 'FAILED',
