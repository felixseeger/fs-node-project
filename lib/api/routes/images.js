...
import * as ernie from '../services/ernie.js';
import * as recraft from '../services/recraft.js';
import * as quiver from '../services/quiver.js';
  ...
  try {
    ...
    switch(true) {
      ...
      case selectedModel === 'Nano Banana 2' ||
        ...
        // Nano Banana variants → Replicate (google/nano-banana-2)
        result = await generateQueue.add(() => replicate.generateImage(req.body));
        break;
      case selectedModel === 'google-imagegen-4':
        // Google Imagegen 4 → Replicate (google/imagen-4)
        result = await generateQueue.add(() => replicate.generateImage(req.body));
        break;
      case selectedModel === 'ideogram-v3-turbo':
        // Ideogram V3 Turbo → Replicate (ideogram-ai/ideogram-v3-turbo)
        result = await generateQueue.add(() => replicate.generateImage(req.body));
        break;
      case selectedModel.includes('Recraft'):
        // Recraft V3/V4 → Recraft API
        result = await generateQueue.add(() => recraft.generateImage({
          prompt: req.body.prompt,
          style: req.body.style || 'any',
      ...
      case selectedModel.includes('Quiver') || selectedModel.includes('quiver'):
        // Quiver → Quiver API
        result = await generateQueue.add(() => quiver.generateImage({
          prompt: req.body.prompt,
          aspect_ratio: req.body.aspect_ratio || '1:1',
          n: req.body.n || 1,
          negative_prompt: req.body.negative_prompt || ''
        ...
        break;
      case selectedModel === 'kling3':
        // KLING 3 → FAL API (image-to-video)
        result = await generateQueue.add(() => fal.generateKlingVideo({
          prompt: req.body.prompt,
          start_image_url: req.body.image,
          duration: req.body.duration || '5',
          generate_audio: req.body.generate_audio !== false
        }));
        break;
      case selectedModel === 'heygen':
        // HeyGen V3 Video-Agent → FAL API (text-to-video)
        result = await generateQueue.add(() => heygen.generateHeyGenVideo({
          prompt: req.body.prompt,
          avatar: req.body.avatar || 'auto',
          voice: req.body.voice || 'auto',
          orientation: req.body.orientation
        }));
        break;
      case selectedModel === 'ernie':
        // Ernie LoRA Turbo → FAL API (text-to-image)
        result = await generateQueue.add(() => ernie.generateImage({
          prompt: req.body.prompt,
          aspect_ratio: req.body.aspect_ratio || '1:1',
          n: req.body.n || 1,
          negative_prompt: req.body.negative_prompt || '',
          guidance_scale: req.body.guidance_scale || 1
        }));
        break;
      default:
