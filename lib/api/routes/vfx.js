...
router.post('/vfx/ltx/generate', async (req, res, next) => {
  try {
    ...
    try {
      const res = await generateLtxVideo(prompt, webhookUrl, { width, height, frames, image_url });
      if (!process.env.FAL_KEY) {
        // Mock successful completion after 5 seconds
        setTimeout(async () => {
          await updateJobStatus(job.id, { 
            status: 'completed', 
            resultUrl: 'http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' 
          });
        }, 5000);
      }
    } catch (error) {
