...
import { getJob, createJob } from '../services/jobTrackerService.js';
      ...
      try {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          await getJob(job.id).then(async j => {
            if (j) {
              const { updateJobStatus } = await import('../services/jobTrackerService.js');
              updateJobStatus(job.id, { progress: i * 20 });
            }
          });
        }
