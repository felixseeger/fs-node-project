...
const oldActions = `  const handleMenuAction = (action, data) => {
          ...
          data: {
            ...
            images: selectedNodes.reduce((acc, node) => {
              if (node.data.outputImage) {
                return [...acc, node.data.outputImage];
              }
              if (node.data.outputVideo) {
                return [...acc, node.data.outputVideo];
              }
              return acc;
            }, []),
          },
...
// Replace the exact old function string
appJsx = appJsx.replace(oldActions, newActions);
fs.writeFileSync('frontend/src/App.jsx', appJsx);
console.log("Patched handleMenuAction");
