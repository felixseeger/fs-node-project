...
// 5. Add TemplateBuilderModal to render
if (!code.includes('<TemplateBuilderModal')) {
  // Find where ProfileModal is rendered in the main return, usually near the end
  code = code.replace(
    /<ProfileModal isOpen=\{isProfileModalOpen\} onClose=\{\(\) => setIsProfileModalOpen\(false\)\} \/>/,
    `<ProfileModal isOpen={isProfileModalOpen} onClose={() => setIsProfileModalOpen(false)} />\n      <TemplateBuilderModal\n        isOpen={showTemplateModal}\n        onClose={() => setShowTemplateModal(false)}\n        selectedNodes={nodes.filter(n => n.selected)}\n        nodes={nodes}\n        edges={edges}\n        onCreated={({ template }) => {\n          setShowTemplateModal(false);\n          // Handle newly created template if needed (e.g. refresh UI)\n        }}\n      />`
  );
}

fs.writeFileSync('frontend/src/App.jsx', code);
console.log('Patched App.jsx for TemplateBuilderModal context menu');
